...
func TestCurrentSessionPaginationAndReopen(t *testing.T) {
	path := filepath.Join(t.TempDir(), "results.db")
	store, err := OpenStore(path)
	if err != nil {
		t.Fatal(err)
	...
	// Saved/imported records that were never started this process stay History-only.
	if err := store.Save(Session{ID: "import", Config: config("https://example.com"), Revision: 1, EndReason: "imported"}, nil); err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 55; i++ {
		s := Session{ID: fmt.Sprintf("current-%02d", i), Config: config("https://example.com"), Revision: 1, Running: true, StartedAt: time.Now()}
		if err := store.Save(s, nil); err != nil {
			t.Fatal(err)
		}
		s.Running, s.Revision, s.EndReason = false, 2, "stopped"
		if err := store.Save(s, nil); err != nil {
			t.Fatal(err)
		}
	}
	page, err := store.List("EXAMPLE", "current", 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Sessions) != 50 || page.Next == 0 || page.Sessions[0].ID != "current-54" {
		t.Fatalf("first page: %+v", page)
	}
	page, err = store.List("EXAMPLE", "current", page.Next)
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Sessions) != 5 || page.Next != 0 || page.Sessions[4].ID != "current-00" {
		t.Fatalf("second page: %+v", page)
	}
	page, err = store.List("absent", "current", 0)
	...
	page, err = store.List("", "current", 0)
	if err != nil || len(page.Sessions) != 0 {
		t.Fatalf("reopened current list: %+v %v", page, err)
	}
	historyCount := 0
	for cursor := int64(0); ; {
		page, err = store.List("", "stopped", cursor)
		if err != nil {
			t.Fatal(err)
		}
		historyCount += len(page.Sessions)
		if page.Next == 0 {
			break
		}
		cursor = page.Next
