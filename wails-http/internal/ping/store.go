...
func (s *Store) Save(test Session, sample *Sample) error {
	...
	if sample != nil {
		...
		_, err = tx.Exec("INSERT INTO samples(test_id,seq,time,data) VALUES(?,?,?,?)", test.ID, sample.Sequence, sample.Time.UnixMilli(), string(point))
		if err != nil {
			return err
		}
		...
		for level, size := 1, int64(4); level <= 20; level, size = level+1, size*4 {
			_, err = statement.Exec(test.ID, level, int64(sample.Sequence-1)/size, 1, good, total, total, total,
...
func (s *Store) ExportCSV(id string, w io.Writer) error {
	...
	if err = writer.Write([]string{"Test ID", "URL", "Method", "Interval (ms)", "Timeout (ms)", "Sequence", "Time (UTC)", "RTT (ms)", "HTTP status", "Success", "Error"}); err != nil {
		return err
	}
	for after := 0; after < test.Sent; {
		...
		for _, p := range batch {
			err = writer.Write([]string{test.ID, test.Config.URL, test.Config.Method, strconv.Itoa(test.Config.IntervalMS), strconv.Itoa(test.Config.TimeoutMS), strconv.Itoa(p.Sequence), p.Time.UTC().Format(time.RFC3339Nano), strconv.FormatFloat(p.RTT, 'f', 3, 64), strconv.Itoa(p.StatusCode), strconv.FormatBool(p.Success), p.Error})
			if err != nil {
				return err
			}
			after = p.Sequence
